...
$selectSrNo = mysqli_query($con, "SELECT * FROM container_setup ORDER BY SrNo DESC LIMIT 1");
...
// fatch data in currency setup
 $selectContainer = mysqli_query($con, "select * from container_setup ");
...
if(isset($_POST["btnDelete"]))
{
  ...
  {
    $selectStatus = mysqli_query($con, "SELECT * FROM container_setup WHERE SrNo='".$val."' ");
    ...
    {
      mysqli_query($con, "UPDATE container_setup SET status='Deactive' WHERE SrNo = '".$val."' ");
    }
     header("Location: container_setup.php");
}
    ...
    if(isset($_POST["btnDelete1"]))
{
  ...
  while (list($key, $val) = @each ($id))
  {
    ...
    {
      mysqli_query($con, "UPDATE container_setup SET status='Active' WHERE SrNo = '".$val."' ");
    }
...
if(isset($_POST['btnedit1']))
{
  ...
  $initQuery = "UPDATE container_setup SET SrNo='$SrNoV' ";
  ...
  $initChangeLog .= " VALUES ('$newID1', 'Container', '$SrNo', '$createBy', '$createDate', '$loginUser', '$todayDate'";
 ...
 if(isset($_POST["btnExport_D"]))
{
  ...
  {
    header("Location: container_excel.php?searchRecord=$searchRecord");
  }
  else if ($exportOptions == "csv")
  {
    echo '<script type="text/javascript" language="Javascript">window.open("container_csv.php?searchRecord=$searchRecord");</script>';
    //header("Location: downloadtableCSV_U.php?searchRecord=$searchRecord");
  }
  else if ($exportOptions == "pdf")
  {
    echo '<script type="text/javascript" language="Javascript">window.open("container_pdf.php?searchRecord=$searchRecord");</script>';
    //header("Location: downloadtableCSV_U.php?searchRecord=$searchRecord");
  }
}
...
//  insert qurey
 $insertQuery = mysqli_query($con, "insert into container_setup (name, status) values ('$name','$status')");

  $insertQuery2 = mysqli_query($con, "insert into chainlog (instance, formName, record_id,createBy, createDate) values ('$newID1', 'Container', '$SrNo1', '$loginUser1', '$todayDate1') ");
...
<html>
<head>
  <title>Container Setup Table</title>
  ...
  <div class="breadCrumb_bar_iner">
    <div class="">
        <div class="btn-group btn-breadcrumb">
          ...
          <a href="container_setup.php" class="btn btn-info active">Container Setup Table</a>
        </div>
    </div>
  </div>
</div>
                  ...
                  <table id="dpttable1" class="display nowrap no-footer" style="width:100%">
                     ...
                     <tbody>
                      <?php
                              ...
                              $selectchainlog = mysqli_query($con, "select * from chainlog where formName = 'Container' ");
                       ...
                       <thead>
                        ...
                        <tbody>
                          ...
                          <?php
                                ...
                                while ($rowContainer= mysqli_fetch_array($selectContainer))
                                {
                                ?>
                        <tr>
                          <?php echo '<td><input type="checkbox" name="user_check[]" value="'. $rowContainer['SrNo'] .' " /></td>'; ?>
                          <td><?php echo $rowContainer['name']; ?></td>
                          <td><?php echo $rowContainer['status']; ?></td>
                          <td><a href="#" class="editData" data-toggle="modal" id="<?php echo $rowContainer['SrNo']; ?>" data-target="#btn1" >Edit</td> 
                           <?php
                          if ($rowContainer['status'] == "Active")
                          {
                          ?>
                          <td><a href="deleteContainer_Code.php?id=<?php echo $rowContainer['SrNo']; ?>" >Deactive</td>
                          <?php
                          }
                          ?>
                          <?php
                          if ($rowContainer['status'] == "Deactive")
                          {
                          ?>
                          <td><a href="deleteContainer_CodeI.php?id=<?php echo $rowContainer['SrNo']; ?>" >Active</td>
                          <?php
                          }
                          ?>
                        </tr>
...
$(document).on('click', '.editData', function(){  
  var employee_id = $(this).attr("id"); 
      ...
      $.ajax({
         url:"fatch_Container.php",  
                method:"GET",  
                data:{employee_id:employee_id},  
                dataType:"json",  
         success: function(data) {
              $('#SrNoV').val(data.SrNo);  
              //$('.nameV').html(data.cur_coun_name);
