...
if(isset($_POST["btnDelete"]))
{
  ...
  {
    $selectStatus = mysqli_query($con, "SELECT * FROM business_setup WHERE SrNo='".$val."' ");
    ...
    {
      mysqli_query($con, "UPDATE business_setup SET status='Deactive' WHERE SrNo = '".$val."' ");
    }
    ...
    if(isset($_POST["btnDelete1"]))
{
  ...
  while (list($key, $val) = @each ($id))
  {
    ...
    {
      mysqli_query($con, "UPDATE business_setup SET status='Active' WHERE SrNo = '".$val."' ");
    }
...
if(isset($_POST['btnedit1']))
{
  ...
  $initQuery = "UPDATE business_setup SET SrNo='$bus_SrNoV' ";
  ...
  $initChangeLog .= " VALUES ('$newID1', 'Business', '$SrNo', '$createBy', '$createDate', '$loginUser', '$todayDate'";

  if ($bus_sec_nameV_n != $bus_sec_nameV_p)
  {
...
// // update qury
//    $updateQuery12 = mysqli_query($con, "UPDATE  business_setup SET bus_sec_name='$bus_sec_nameV',commodities='$commoditiesV',status='$statusV' WHERE SrNo='$bus_SrNoV' ") or die(mysqli_error($con));
 ...
 if(isset($_POST["btnExport_D"]))
{
  ...
  {
    header("Location: bus_excel.php?searchRecord=$searchRecord");
  }
  else if ($exportOptions == "csv")
  {
    echo '<script type="text/javascript" language="Javascript">window.open("bus_csv.php?searchRecord=$searchRecord");</script>';
    //header("Location: downloadtableCSV_U.php?searchRecord=$searchRecord");
  }
  else if ($exportOptions == "pdf")
  {
    echo '<script type="text/javascript" language="Javascript">window.open("bus_pdf.php?searchRecord=$searchRecord");</script>';
    //header("Location: downloadtableCSV_U.php?searchRecord=$searchRecord");
  }
}
...
if (isset($_POST['btnadd'])) {
  ...
  $bus_sec_name = $_POST['bus_sec_name'];
...
//  insert qurey
 $insertQuery = mysqli_query($con, "insert into business_setup (bus_sec_name,commodities,status) values ('$bus_sec_name','$commodities','$status')");
...
<html>
<head>
  <title>Business Setup Table</title>
  ...
  <div class="breadCrumb_bar_iner">
    <div class="">
        <div class="btn-group btn-breadcrumb">
          ...
          <a href="business_setup_table.php" class="btn btn-info active">Business Setup Table</a>
        </div>
    </div>
  </div>
</div>
                  ...
                  <div class="input-fields">  
                   ...
                   <div class="input-fields"> 
                    <label>Commodity</label> 
                     <select name="commodities" id="commodities" class="commodities" required>
                  ...
                  <table id="dpttable1" class="display nowrap no-footer" style="width:100%">
                     ...
                     <tbody>
                      <?php
                              ...
                              $selectchainlog = mysqli_query($con, "select * from chainlog where formName = 'Business' ");
...
<script type="text/javascript">
  ...
  var commodities = document.getElementById("commodities").value;
      ...
      $.ajax({
         url:"addMoreBusiness.php",
         method:"GET",
         data:{bus_sec_name:bus_sec_name, commodities:commodities},
         dataType:"text",
         success: function(data) {
              
              alert("Done");
              document.getElementById("bus_sec_name").value = "";
