...
if(isset($_POST["btnDelete"]))
{
  ...
  {
    $selectStatus = mysqli_query($con, "SELECT * FROM stockowner WHERE SrNo='".$val."' ");
    ...
    {
      mysqli_query($con, "UPDATE stockowner SET status='Deactive' WHERE SrNo = '".$val."' ");
    }
    ...
    if(isset($_POST["btnDelete1"]))
{
  ...
  while (list($key, $val) = @each ($id))
  {
    ...
    {
      mysqli_query($con, "UPDATE stockowner SET status='Active' WHERE SrNo = '".$val."' ");
    }
 ...
 if(isset($_POST["btnExport_D"]))
{
  ...
  {
    header("Location:  stockowner_excel.php?searchRecord=$searchRecord");
  }
  else if ($exportOptions == "csv")
  {
    echo '<script type="text/javascript" language="Javascript">window.open(" stockowner_csv.php?searchRecord=$searchRecord");</script>';
    //header("Location: downloadtableCSV_U.php?searchRecord=$searchRecord");
  }
  else if ($exportOptions == "pdf")
  {
    echo '<script type="text/javascript" language="Javascript">window.open(" stockowner_pdf.php?searchRecord=$searchRecord");</script>';
    //header("Location: downloadtableCSV_U.php?searchRecord=$searchRecord");
  }
}
  ...
  <div class="breadCrumb_bar_iner">
    <div class="">
        <div class="btn-group btn-breadcrumb">
          ...
          <a href="usertable.php" class="btn btn-info active">Stock owner Table</a>
        </div>
    </div>
  </div>
</div>
    ...
    <div class="">
      <div class="col-md-12">
        <div class="user_create_btn">
          ...
          <button type="button" id="btnNewUser" onclick="newforeignassociate();">Add Stock owner</button>
        </div>
      </div>

      <div class="col-md-12">
        <div class="user_table-title">
          <h4>Stock owner Table</h4>
        </div>
                              ...
                              <?php
                                   ...
                                   // fetch data in currency setup
                                    ...
                                    while ($rowtablevar= mysqli_fetch_array($selectvar))
                                    {
                                      ...
                                      $owner_name = $rowtablevar['owner_name'];
                                      $owner_add = $rowtablevar['owner_add'];
                                      $owner_country = $rowtablevar['owner_country'];
                                      $owner_city = $rowtablevar['owner_city'];
                                      $owner_phone = $rowtablevar['owner_phone'];
                                      $fax = $rowtablevar['fax'];
                                      $email = $rowtablevar['email'];
                                      $website = $rowtablevar['website'];
                                      $iata_code = $rowtablevar['iata_code'];
                                      $commesion = $rowtablevar['commesion'];
                                      $wht = $rowtablevar['wht'];
                                    ...
                                    ?>        
                                      ...
                                      <td><?php echo $owner_add; ?></td>
                                      <td><?php echo $owner_country; ?></td>
                                      <td><?php echo $owner_city; ?></td>
                                      <td><?php echo $owner_phone; ?></td>
                                      <td><?php echo $fax; ?></td>
                                      <td><?php echo $email; ?></td>
                                      <td><?php echo $website; ?></td>
                                      <td><?php echo $iata_code ?></td>
                                      <td><?php echo $commesion ?></td>
                                      <td><?php echo $wht ?></td>
                                      <td><?php echo $status; ?></td>
                                      
                                      <td><a href=" stock_owner_air_line_finance_code_View.php?id=<?php echo $rowtablevar['SrNo']; ?>">View</td>
                                      <td><a href=" stock_owner_air_line_finance_code_Edit.php?id=<?php echo $rowtablevar['SrNo']; ?>">Edit</td>
                                       <?php
                                        if ($rowtablevar['status'] == "Active")
                                        {
                                        ?>
                                        <td><a href="deleteStockCode.php?id=<?php echo $rowtablevar['SrNo']; ?>" >Deactive</td>
                                        <?php
                                        }
                                        ?>
                                        ...
                                        {
                                        ?>
                                        <td><a href="deleteStockCodeI.php?id=<?php echo $rowtablevar['SrNo']; ?>" >Active</td>
                                        <?php
                                        }
                                        ?> 
...
function newforeignassociate() {
  location.replace(" stock_owner_air_line_finance_code.php")
}
</script>
