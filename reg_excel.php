...
include 'manage/connection.php';
 ...
 $selectRole =  "select * from  region_setup ";
...
$iii = 2;
while ($rowRole = mysqli_fetch_array($select))
{
   
    $reg_code = $rowRole['reg_code'];
    $reg_name = $rowRole['reg_name'];
    $status = $rowRole['status'];
    ...
    $objPHPExcel ->getActiveSheet() ->setCellValue('A1', 'Region Code')
    ->setCellValue('B1', 'Region Name')
    ->setCellValue('C1', 'Status');

    $objPHPExcel ->getActiveSheet() ->setCellValue($a, $reg_code)
    ->setCellValue($b, $reg_name)
