...
$vndID = $_GET['vndID'];
$qry= "SELECT * FROM vndmaster WHERE newCode = '$vndID'";
...
if ($vndID==$row['newCode'])
{
  ...
  $vndTitle_p = $row['vndTitle'];
  $vndStreet_p = $row['vndStreet'];
  $vndCity_p = $row['vndCity'];
  $vndCountry_p = $row['vndCountry'];
...
if(isset($_POST['updateBtn']))
{
	...
	$initQuery = "UPDATE vndmaster SET legCode='$legCode' ";

	if ($vndTitle != $vndTitle_p)
	{
		$initQuery .= ", vndTitle='$vndTitle'";
	}
	if ($vndStreet != $vndStreet_p)
	{
		$initQuery .= ", vndStreet='$vndStreet'";
	}
	if ($vndCity != $vndCity_p)
	{
		$initQuery .= ", vndCity='$vndCity'";
	}
	if ($vndCountry != $vndCountry_p)
	{
		$initQuery .= ", vndCountry='$vndCountry'";
	}
	...
	$updateQuery = mysqli_query($con, $initQuery);

	header("Location: master_vendor_VE2.php?vndID=" . $vndID);



	/*$updateQuery =  mysqli_query($con, "UPDATE vndmaster SET legCode='$legCode', vndTitle='$vndTitle', vndStreet='$vndStreet', vndCity='$vndCity', vndCountry='$vndCountry', telCode='$telCode', telNumber='$telNumber', cmpWebsite='$cmpWebsite', cmpEmail='$cmpEmail', taxType='$taxType', taxNo='$taxNo', seaImport='$seaImport', airImport='$airImport', seaExport='$seaExport', airExport='$airExport', cmpStatus='1' WHERE newCode='$vndID' ");*/
