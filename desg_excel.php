...
include 'manage/connection.php';
 ...
 $selectdesg =  "select * from  designation ";
$select = mysqli_query($con, $selectdesg);
...
$iii = 2;
while ($rowdesg = mysqli_fetch_array($select))
{
   
    $Desig_ID = $rowdesg['Desig_ID'];
    $Desig_name = $rowdesg['Desig_name'];
    $savedStatus = $rowdesg['savedStatus'];
    ...
    $objPHPExcel ->getActiveSheet() ->setCellValue('A1', 'Designation ID')
    ->setCellValue('B1', 'Designation Name')
    ->setCellValue('C1', 'Status');

    $objPHPExcel ->getActiveSheet() ->setCellValue($a, $Desig_ID)
    ->setCellValue($b, $Desig_name)
