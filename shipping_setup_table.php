...
// fetch data in shippingline setup
...
{
  // $empNo = $_POST['empNo'];
  $ship_codeV = $_POST['ship_codeV'];
  $ship_nameV = $_POST['ship_nameV'];
  $ship_SrNoV = $_POST['ship_SrNoV'];
  ...
  else
  {
   ...
   $updateQuery12 = mysqli_query($con, "UPDATE shipping_setup SET ship_code='$ship_codeV', ship_name='$ship_nameV',status='$statusV' WHERE SrNo='$ship_SrNoV' ") or die(mysqli_error($con));
...
if (isset($_POST['btnadd'])) {
 ...
 $insertQuery = mysqli_query($con, "insert into shipping_setup (ship_code, ship_name,status) values ('$ship_code', '$ship_name', '$status')");
...
<html>
<head>
  <title>Shipping Line Setup Table</title>
  ...
  <div class="breadCrumb_bar_iner">
    <div class="">
        <div class="btn-group btn-breadcrumb">
          ...
          <a href="shipping_setup_table.php" class="btn btn-info active">Shipping Line Setup Table</a>
        </div>
    </div>
  </div>
</div>
...
<div class="leave-manage-sec  main_widget_box ">
      ...
      <div class="form_sec_action_btn col-md-12">
          ...
          <button type="button" id="myBtn">  <small>Add Shipping Line</small></button>
      </div>
            ...
            <div class="modal-dialog">
              ...
              <div class="modal-content">
                <div class="modal-header">
                  ...
                  <h4 class="modal-title">Shipping Line Details</h4>
                </div>
                <div class="modal-body">
                   <div class="input-fields"> 
                    <label>Shipping Line Code</label> 
                    <input type="text" name="ship_code" id="ship_code" placeholder="Enter Here Shipping Line Code!">    
                  </div>

                  <div class="input-fields">  
                    <label>Shipping Line Name</label> 
                    <input type="text" name="ship_name" id="ship_name" placeholder="Enter Here Shipping Line Name !">    
                  </div>
                  <div class="input-fields">  
                    <label>Active</label> 
      ...
      <div class="modal fade symfra_popup2" id="btn1" role="dialog">
            <div class="modal-dialog">
              ...
              <div class="modal-content">
                <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal">&times;</button>
                  <h4 class="modal-title">Edit Shipping Line</h4>
                </div>
                <div class="modal-body">
                  <div class="input-fields hide"> 
                    <label>Shipping Line SrNo</label> 
                    <input type="text" name="ship_SrNoV" id="ship_SrNoV" >
                       
                  </div>
                   <div class="input-fields"> 
                    <label>Shipping Line Code</label> 
                    <input type="text" name="ship_codeV" id="ship_codeV" >
                       
                  </div>

                  <div class="input-fields">  
                    <label>Shipping Line Name</label> 
                    <input type="text" name="ship_nameV" id="ship_nameV" >   
                  </div>
                  <div class="input-fields">  
                    <label>Active</label> 
        ...
        <div class="tbleDrpdown">
             <div id="tblebtn">
                <ul>
                  ...
                  <li><button type="submit" id="btnExport_P"> <i class="fa fa-print"></i><a href="shipping_print.php" target="_blank"> Print</a></button></li>
                       ...
                       <thead>
                        ...
                        <tbody>
                          ...
                          <?php
                                ...
                                ?>
                        <tr>
                          <?php echo '<td><input type="checkbox" name="user_check[]" value="'. $rowship['ship_code'] .' " /></td>'; ?>
                          <td><?php echo $rowship['ship_code']; ?></td>
                          <td><?php echo $rowship['ship_name']; ?></td>
                          <td><?php echo $rowship['status']; ?></td>
                          <td><a href="#" class="editData" data-toggle="modal" id="<?php echo $rowship['ship_code']; ?>" data-target="#btn1" >Edit</td> 
                          <?php
                          if ($rowship['status'] == "Active")
                          {
                          ?>
                          <td><a href="deleteShip_Code.php?id=<?php echo $rowship['SrNo']; ?>" >Deactive</td>
                          <?php
                          }
                          ?>
                          <?php
                          if ($rowship['status'] == "Deactive")
                          {
                          ?>
                          <td><a href="deleteShip_CodeI.php?id=<?php echo $rowship['SrNo']; ?>" >Active</td>
                          <?php
                          }
                          ?>
                        </tr>
...
$(document).on('click', '.editData', function(){  
  var employee_id = $(this).attr("id"); 
      ...
      $.ajax({
         url:"fatch_shipping.php",  
                method:"GET",  
                data:{employee_id:employee_id},  
                dataType:"json",  
         success: function(data) {
              $('#ship_SrNoV').val(data.SrNo);  
              $('#ship_codeV').val(data.ship_code);  
              $('#ship_nameV').val(data.ship_name);  
