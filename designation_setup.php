...
// fatch data in currency setup
 $selectRole = mysqli_query($con, "select * from  designation ");
...
if(isset($_POST["btnDelete"]))
{
  ...
  {
    $selectStatus = mysqli_query($con, "SELECT * FROM designation WHERE Desig_ID='".$val."' ");
    while ($rowStatus = mysqli_fetch_array($selectStatus))
    {
      $currentStatus = $rowStatus['savedStatus'];
    }
    if ($currentStatus == "Active")
    {
      mysqli_query($con, "UPDATE designation SET savedStatus='Deactive' WHERE Desig_ID = '".$val."' ");
    }
    ...
    if(isset($_POST["btnDelete1"]))
{
  ...
  while (list($key, $val) = @each ($id))
  {
    ...
    {
      mysqli_query($con, "UPDATE designation SET savedStatus='Active' WHERE Desig_ID = '".$val."' ");
    }
...
if(isset($_POST['btnedit1']))
{
  ...
  $Desig_IDV= $_POST['Desig_IDV'];
  $Desig_nameV = $_POST['Desig_nameV'];

   if (isset($_POST['savedStatusVV'])) {
    $savedStatusV='Active';

  }
  else
  {
    $savedStatusV='Deactive';
  }
 

// update qury
   $updateQuery12 = mysqli_query($con, "UPDATE designation SET Desig_name='$Desig_nameV',savedStatus='$savedStatusV' WHERE Desig_ID='$Desig_IDV' ") or die(mysqli_error($con));
 ...
 if(isset($_POST["btnExport_D"]))
{
  ...
  {
    header("Location: desg_excel.php?searchRecord=$searchRecord");
  }
  else if ($exportOptions == "csv")
  {
    echo '<script type="text/javascript" language="Javascript">window.open("desg_csv.php?searchRecord=$searchRecord");</script>';
    //header("Location: downloadtableCSV_U.php?searchRecord=$searchRecord");
  }
  else if ($exportOptions == "pdf")
  {
    echo '<script type="text/javascript" language="Javascript">window.open("desg_pdf.php?searchRecord=$searchRecord");</script>';
    //header("Location: downloadtableCSV_U.php?searchRecord=$searchRecord");
  }
}
...
if (isset($_POST['btnadd'])) {
  ...
  if (isset($_POST['savedStatus'])) {
    $savedStatus='Active';

  }
  else
  {
    $savedStatus='Deactive';
  }
//  insert qurey
 $insertQuery = mysqli_query($con, "insert into designation (Desig_ID,Desig_name,savedStatus) values ('$Desig_ID','$Desig_name','$savedStatus')");
...
<html>
<head>
  <title>Designation Setup Table</title>
  ...
  <div class="breadCrumb_bar_iner">
    <div class="">
        <div class="btn-group btn-breadcrumb">
          ...
          <a href="hr_add_emp_info.php" class="btn btn-info active">Designation Setup Table</a>
        </div>
    </div>
  </div>
</div>
      ...
      <div class="form_sec_action_btn col-md-12">
        <!-- 
         ...
         -->
          ...
          <button type="button" id="myBtn">  <small>Add Designation</small></button>
      </div>
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                  ...
                  <div class="input-fields">  
                   ...
                   <?php echo '<input type="text" name="Desig_ID" id="Desig_ID" disabled value="'.$Desig_ID.'">'; ?>    
                  </div>
                   <div class="input-fields"> 
                    <label>Designation Name</label> 
                    <input type="text" name="Desig_name" id="Desig_name" placeholder="Enter Here Designation Name!">    
                  </div>
                   <div class="input-fields">  
                    <label>Active</label> 
                    <input type="checkbox" name="savedStatus" id="savedStatus">    
                  </div>
      ...
      <div class="modal fade symfra_popup2" id="btn1" role="dialog">
            <div class="modal-dialog">
              ...
              <div class="modal-content">
                <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal">&times;</button>
                  <h4 class="modal-title">Edit Designation Details</h4>
                </div>
                <div class="modal-body">
                  <div class="input-fields hide" > 
                    <label>Designation ID</label> 
                    <input type="text" name="role_idV" id="role_idV">
                       
                  </div>

                  <div class="input-fields">  
                    <label>Designation Name</label> 
                    <input type="text" name="Desig_nameV" id="Desig_nameV" placeholder="Enter Here Designation Code !">    
                  </div>
                  
                   <div class="input-fields">  
                    <label>Active</label> 
                    <input type="checkbox" name="savedStatusV" id="savedStatusV">    
                  </div>
        ...
        <div class="tbleDrpdown">
             <div id="tblebtn">
                <ul>
                  ...
                  <li><button type="submit" id="btnExport_P"> <i class="fa fa-print"></i><a href="desg_print.php" target="_blank"> Print</a></button></li>
                       ...
                       <thead>
                        ...
                        <tbody>
                          ...
                          <?php
                                ...
                                ?>
                        <tr>
                          <?php echo '<td><input type="checkbox" name="user_check[]" value="'. $rowRole['Desig_ID'] .' " /></td>'; ?>
                          <td><?php echo $rowRole['Desig_ID']; ?></td>
                          <td><?php echo $rowRole['Desig_name']; ?></td>
                          <td><?php echo $rowRole['savedStatus']; ?></td>
                          <td><a href="#" class="editData" data-toggle="modal" id="<?php echo $rowRole['Desig_ID']; ?>" data-target="#btn1" >Edit</td> 
                           <?php
                          if ($rowRole['savedStatus'] == "Active")
                          {
                          ?>
                          <td><a href="deleteDesg_Code.php?id=<?php echo $rowRole['Desig_ID']; ?>" >Deactivate</td>
                          <?php
                          }
                          ?>

                          <?php
                          if ($rowRole['savedStatus'] == "Deactive")
                          {
                          ?>
                          <td><a href="deleteDesg_CodeI.php?id=<?php echo $rowRole['Desig_ID']; ?>" >Activate</td>
                          <?php
                          }
                          ?>
                        </tr>
  ...
  var employee_id = $(this).attr("id"); 
      ...
      $.ajax({
         ...
         success: function(data) {
              $('#Desig_IDV').val(data.Desig_ID);  
              //$('.cur_coun_nameV').html(data.cur_coun_name);
              $('#Desig_nameV').val(data.Desig_name);   

              var checkif = data.savedStatus;
              if (checkif == "Active") {
                 $('#savedStatusV').attr("checked", true);
                 document.getElementByID("savedStatusV").checked = true;
              }
              else
              {
                $('#savedStatusV').attr("checked", false);
              }
